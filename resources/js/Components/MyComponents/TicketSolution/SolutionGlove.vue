<template>
    <div @click="optionsDropdown = false" class="flex space-x-5 my-4 relative">
        ...
        <div
          ...
          class="border border-gray5 flex flex-col space-y-1 rounded-md py-2 absolute w-28 h-auto top-10 right-4 text-sm z-50 bg-white"
        >
          <p
            @click="editPublication = true"
            class="hover:bg-pink-100 cursor-pointer text-center px-2"
          >
            Editar
          </p>
          <el-popconfirm confirm-button-text="Si" cancel-button-text="No" icon-color="#ff4d4d"
                ...
                <template #reference>
                    <p @click.stop class="hover:bg-pink-100 cursor-pointer text-center px-2">Eliminar</p>
                </template>
            </el-popconfirm>
      </div>
        ...
        <div class="w-full border border-grayD9 rounded-3xl py-3 px-9 mt-2 text-sm">
            ...
            <div class="flex justify-between items-center">
                ...
                <p class="text-gray66 text-sm">{{ solution.created_at['isoFormat'] }}<span class="ml-2">({{ solution?.created_at['diffForHumans'] }})</span></p>
            </div>
            <p>{{ solution.description }}</p>
            <figure v-for="file in solution.media" :key="file">
                <img  class="object-cover w-32" :src="file.original_url" alt="">
            </figure>
        </div>
    </div>
</template>
