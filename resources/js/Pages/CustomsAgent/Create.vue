<template>
    <AppLayout title="Crear agente aduanal">
        <main class="px-2 lg:px-14 py-8">
            <form @submit.prevent="store" class="rounded-lg border border-gray-200 lg:p-5 p-3 lg:w-1/2 mx-auto">
                <div class="flex items-center space-x-2 mb-6">
                    <Back />
                    <h1 class="font-semibold text-xl text-gray-800">Crear nuevo agente aduanal</h1>
                </div>
...
export default {
    ...
    methods: {
        store() {
            this.form.post(route('customs-agents.store'), {
                onSuccess: () => this.$notify({ title: 'Éxito', message: 'Agente aduanal creado', type: 'success' }),
            });
        },
    },
};
